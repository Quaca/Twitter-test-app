...
import com.example.twitter.repository.FollowershipRepository;
...
public class UserService {
    ...
    public UserService(KeycloakService keycloakService, FollowershipRepository followershipRepository, PasswordEncoder passwordEncoder, MessageSource messages,
    ...
    public User followUser(User fromUser, String toUserId) {
        ...
        List<Followership> following = followershipRepository.findByFromId(fromUser.getId());
        fromUser.setFollowing(following.stream().map(followership -> followership.getToId()).collect(Collectors.toList()));

        UserRepresentation userRepresentation = keycloakService.findUserById(toUserId);
        FollowershipId followershipId = new FollowershipId(fromUser.getId(), toUserId);

        if (fromUser.getFollowing().contains(toUserId)) {
            throw new ResourceAlreadyExistingException("User already follows the specified user.", "#FollowershipAlreadyExisting");
        }

        Followership followership = new Followership(followershipId);
        followershipRepository.save(followership);
        fromUser.addFollowing(followership.getToId());
        return fromUser;
    }

    @Transactional
    public void unfollowUser(User fromUser, String toUserId) {

        Followership followership = followershipRepository.findByFromIdAndToId(fromUser.getId(), toUserId).orElseThrow(
                () -> new NoResourceException("User does not follow the specified user", "#FollowershipNotExisting")
        );
